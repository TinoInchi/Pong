#define is_down(b) input->buttons[b].is_down
#define pressed(b) (input->buttons[b].is_down && input->buttons[b].changed)
#define released(b) (!input->buttons[b].is_down && input->buttons[b].changed)

float player_1_pos_y,player_1_dp,player_2_pos_y, player_2_dp;
float arena_half_size_x = 100, arena_half_size_y = 50;
float player_half_size_x = 1, player_half_size_y = 10;
float ball_p_x, ball_p_y, ball_dp_x = 100, ball_dp_y = 0, ball_half_size = 1;

int player_1_score, player_2_score;



internal_variable void simulate_player(float *p, float *dp, float ddp, float dt) {


	ddp -= *dp * 30.f;

	*dp = *dp + ddp * dt;
	*p = *p + *dp * dt + .5f * ddp * dt * dt;

	if (*p + player_half_size_y > arena_half_size_y) {
		*p = arena_half_size_y - player_half_size_y;
		*dp = 0;
	}
	else if (*p - player_half_size_y < -arena_half_size_y) {
		*p = -arena_half_size_y + player_half_size_y;
		*dp = 0;
	}
}

internal_variable bool aabb_vs_aabb(float p1x, float p1y, float hs1x, float hs1y,
	float p2x, float p2y, float hs2x, float hs2y) {

	return (p1x + hs1x > p2x - hs2x &&
		p1x - hs1x < p2x + hs2x &&
		p1y + hs1y > p2y - hs2y &&
		p1y + hs1y < p2y + hs2y);
}

enum Gamemode {
	GM_MENU,
	GM_GAMEPLAY,
};

Gamemode current_gamemode;
int hot_button;
bool enemy_is_ai;

internal_variable void
simulate_game(Input* input, float dt) {
	clear_screen(0x000000);



	if (current_gamemode == GM_GAMEPLAY) {

		if (pressed(BUTTON_ESC)) {
			current_gamemode = GM_MENU;
			player_1_pos_y = 0.f;
			player_2_pos_y = 0.f;
			player_1_score = 0;
			player_2_score = 0;
			ball_p_x = 0.f;
			ball_p_y = 0.f;
			ball_dp_y = 0.f;

		}

		float player_1_ddp = 0.f;
		if (!enemy_is_ai) {
			if (is_down(BUTTON_UP)) player_1_ddp += 2500;
			if (is_down(BUTTON_DOWN)) player_1_ddp -= 2500;
		}
		else {
			//player_1_ddp = (ball_dp_y - player_1_pos_y) * 100;
			if (ball_p_y > player_1_pos_y) player_1_ddp += 4500;
			if (ball_p_y < player_1_pos_y) player_1_ddp -= 4500;
			//if (player_1_ddp > 4500) player_1_ddp = 4500;
			//if (player_1_ddp < -4500) player_1_ddp = -4500;
		}
		float player_2_ddp = 0.f;
		if (is_down(BUTTON_W)) player_2_ddp += 3000;
		if (is_down(BUTTON_S)) player_2_ddp -= 3000;

		simulate_player(&player_1_pos_y, &player_1_dp, player_1_ddp, dt);
		simulate_player(&player_2_pos_y, &player_2_dp, player_2_ddp, dt);

		// Simulate Ball
		{
			ball_p_x += ball_dp_x * dt;
			ball_p_y += ball_dp_y * dt;


			if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 85, player_1_pos_y, player_half_size_x, player_half_size_y)) {
				ball_p_x = 85 - player_half_size_x - ball_half_size;
				ball_dp_x *= -1;
				ball_dp_y = (ball_p_y - player_1_pos_y) * 5 + player_1_dp * 1.5f;
			}
			else if (aabb_vs_aabb(ball_p_x, ball_p_y, ball_half_size, ball_half_size, -85, player_2_pos_y, player_half_size_x, player_half_size_y)) {
				ball_p_x = -85 + player_half_size_x + ball_half_size;
				ball_dp_x *= -1;
				ball_dp_y = (ball_p_y - player_2_pos_y) * 5 + player_2_dp * 1.5f;
			}


			if (ball_p_y + ball_half_size > arena_half_size_y) {
				ball_p_y = arena_half_size_y - ball_half_size;
				ball_dp_y *= -1;
			}
			else if (ball_p_y + ball_half_size < -arena_half_size_y) {
				ball_p_y = -arena_half_size_y + ball_half_size;
				ball_dp_y *= -1;
			}


			if (ball_p_x + ball_half_size > arena_half_size_x) {
				ball_p_x = 0;
				ball_p_y = 0;
				ball_dp_x *= -1;
				ball_dp_y = 0;
				player_1_score++;
			}
			else if (ball_p_x - ball_half_size < -arena_half_size_x) {
				ball_p_x = 0;
				ball_p_y = 0;
				ball_dp_x *= -1;
				ball_dp_y = 0;
				player_2_score++;
			}
		}

		draw_number(player_1_score, -10, 40, 1.f, 0xffffff);
		draw_number(player_2_score, 10, 40, 1.f, 0xffffff);

		draw_rect(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 0xffffff);
		draw_rect(-85, player_2_pos_y, player_half_size_x, player_half_size_y, 0xffffff);
		draw_rect(85, player_1_pos_y, player_half_size_x, player_half_size_y, 0xffffff);
	}
	else {

		if (pressed(BUTTON_LEFT) || pressed(BUTTON_RIGHT)) {
			hot_button = !hot_button;
		}

		if (pressed(BUTTON_ENTER)) {
			current_gamemode = GM_GAMEPLAY;
			enemy_is_ai = hot_button ? 0 : 1;
		}

		if (pressed(BUTTON_ESC)) {
			running = !running;
		}

		if (hot_button == 0) {
			draw_rect(20, 0, 15, 15, 0xffffff);
			draw_rect(-20, 0, 20, 20, 0xffffff);
			menu_directory_naming(false, 20, 0, 2.5f, 0x000000);
			menu_directory_naming(true, -20, 0, 2.5f, 0x000000);
		}
		else {
			draw_rect(20, 0, 20, 20, 0xffffff);
			draw_rect(-20, 0, 15, 15, 0xffffff);
			menu_directory_naming(false, 20, 0, 2.5f, 0x000000);
			menu_directory_naming(true, -20, 0, 2.5f, 0x000000);
		}
	}
}